<?php
	...
	session_start();
	error_reporting(0);
	$usernameAlt = $_POST['username'];
	$senhaAlt = $_POST['senha-alt'];
	$emailAlt = $_POST['e-mail-alt'];
	$telAlt = $_POST['tel-alt'];
	$cadAlt = false;
	$b = 0;
	while($b <= $i){
		$b++;
		if(($usernameAlt == $users[$b] || $emailAlt == $email[$b] || $telAlt == $telefone[$b]) && ($_SESSION['nome'] != $usernameAlt || $_SESSION['email'] != $emailAlt || $_SESSION['tel'] != $telAlt)){
			$cadAlt = true;
		}
	}
	if($cadAlt != true){
		$alt = "UPDATE cliente SET user= '$usernameAlt' where user='". $_SESSION['nome']. "'"; 
		$con = $mysqli->query($alt) or die($mysqli->error);
		$alt = "UPDATE cliente SET senha= '$senhaAlt' where senha='". $_SESSION['senha']. "'";
		$con = $mysqli->query($alt) or die($mysqli->error);
		$alt = "UPDATE cliente SET telefone= '$telAlt' where telefone='". $_SESSION['tel']. "'";
		$con = $mysqli->query($alt) or die($mysqli->error);
		$alt = "UPDATE cliente SET email= '$emailAlt' where email='". $_SESSION['email']. "'";
		$con = $mysqli->query($alt) or die($mysqli->error);
		$_SESSION['nome'] = $usernameAlt;
		$_SESSION['email'] = $emailAlt;
		$_SESSION['tel'] =$telAlt;
		$_SESSION['senha'] = $senhaAlt;
		setcookie('nome', $usernameAlt);
	}
	else{
	?>
	<script>alert("O nome, o e-mail ou o telefone inserido já existem no sistema!"); location.assign("pagPerfil.php");</script>
	<?php
	}
?>
<!DOCKTYPE html>
...
<div id="areaUser">
	...
	<br>
	<br>
	<p id="boasVindas">Olá, <?php echo $_SESSION['nome']; ?>!</p>
	<br>
	<br>
