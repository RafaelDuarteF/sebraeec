...
if($_SESSION['logWithCookiesAssoc'] == true){
    ?>
   <div id="logWithCook">Último acesso em: <?php echo $_COOKIE['Acess']; ?></div>
 <?php
    $_SESSION['logWithCookiesAssoc'] = false;
}
setcookie('Acess', date('d-m-Y h:i:s A', time()), time()+432000);
$compCod = $mysqli->query("UPDATE compra SET statusCompra = 'Preparando' WHERE cpfAssoc = '".$_SESSION['cpfAssoc']."' and prazo - CURRENT_DATE > 15") or die ($mysqli->error);
$compCod = $mysqli->query("UPDATE compra SET statusCompra = 'A enviar' WHERE cpfAssoc = '".$_SESSION['cpfAssoc']."' and prazo - CURRENT_DATE <= 15 and prazo - CURRENT_DATE > 10") or die ($mysqli->error);
$compCod = $mysqli->query("UPDATE compra SET statusCompra = 'Enviado' WHERE cpfAssoc = '".$_SESSION['cpfAssoc']."' and prazo - CURRENT_DATE <= 10 and prazo - CURRENT_DATE > 5") or die ($mysqli->error);
$compCod = $mysqli->query("UPDATE compra SET statusCompra = 'Em trânsito' WHERE cpfAssoc= '".$_SESSION['cpfAssoc']."' and prazo - CURRENT_DATE <= 5 and prazo - CURRENT_DATE > 0") or die ($mysqli->error);
$compCod = $mysqli->query("UPDATE compra SET statusCompra = 'Entregue' WHERE cpfAssoc = '".$_SESSION['cpfAssoc']."' and prazo - CURRENT_DATE <= 0") or die ($mysqli->error);
...
<head>
	...
	<title>SEBRAEEC - <?php echo $_SESSION['nomeAssoc']; ?></title>
